        ...
        monthRange: {
          ...
          Views_MonthGrid = {
            ...
            destroy: function () {
                  ...
                  self.inputElem.on("change", function(e) {
                      var date = new pDate($(this).val().split('/').map(function(a) {
                        return +a;
                      }));
                      if(date.gDate == "Invalid Date") return;
                      self._updateState("unix", date.valueOf(), true);
                      self.updateStaff();
                  });
                  var day = 24*60*60*1000;
                  self.inputElem.on("keydown", function(e) {
                    if(e.keyCode == 37) {
                      e.preventDefault();
                      self._updateState("unix", self.state.unixDate + day, true);
                      self.updateStaff();
                    }
                    if(e.keyCode == 38) {
                      e.preventDefault();
                      self._updateState("unix", self.state.unixDate - day*7, true);
                      self.updateStaff();
                    }
                    if(e.keyCode == 39) {
                      e.preventDefault();
            ...
            _defineCurrentState: function () {
                  if (this.inputElem.val() && new Date(this.inputElem.val()) != "Invalid Date" && new Date(this.inputElem.val()) != "undefined") {
                        var arr = this.inputElem.val().split('/').map(function(a) {
                          return +a;
                        });
                        this.state.unixDate = (new pDate(arr)).gDate.valueOf();
                  } else {
